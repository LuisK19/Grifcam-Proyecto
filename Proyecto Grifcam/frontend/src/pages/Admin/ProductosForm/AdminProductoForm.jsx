...
// Cada imagen en el estado: { id, src (URL), file (File|null), esExistente }
let nextImgId = 1
...
export default function AdminProductoForm() {
  ...
  // Carga datos del producto si es edición
  useEffect(() => {
    if (!esEdicion) return
    ...
    async function cargarProducto() {
      ...
      try {
        ...
        setForm({
          ...
          is_offer:       producto.is_offer ?? false,
          is_new:         producto.is_new ?? false,
        ...
        setErrores({ global: 'No se pudo cargar el producto.' })
      } finally {
        setCargando(false)
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (form.price_before) formData.append('previous_price', form.price_before)
      ...
      // Solo adjunta las imágenes nuevas (file !== null)
      imagenes
      ...
      setErrores({ global: msg || 'Ocurrió un error. Intentá de nuevo.' })
    } finally {
      setGuardando(false)
    }
  }

  // ── Estado de carga inicial (solo en edición) ──
  if (cargando) {
    return (
      <AdminLayout>
        <div className={styles.page}>
          <p style={{ padding: '3rem', opacity: 0.5 }}>Cargando producto...</p>
        </div>
      </AdminLayout>
    )
  }
    ...
    <AdminLayout>
      <div className={styles.page}>
        ...
        <form className={styles.form} onSubmit={handleSubmit} noValidate>
          ...
          {/* SECCIÓN IMÁGENES — ancho completo */}
          <div className={styles.seccion}>
