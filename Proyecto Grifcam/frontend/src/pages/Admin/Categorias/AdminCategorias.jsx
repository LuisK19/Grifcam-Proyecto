...
export default function AdminCategorias() {
  ...
  useEffect(() => {
    async function cargar() {
      try {
        ...
        // El backend devuelve { id, name }. La cantidad de productos no viene en este endpoint,
        // la inicializamos en 0 hasta que exista un endpoint que la incluya.
        setCategorias(res.data.map(c => ({ ...c, cantidad: 0 })))
      } catch (err) {
  ...
  // NOTA: crear/editar/eliminar categorías se manejan localmente por ahora
  // porque el adapter actual solo tiene endpoints GET para categorías.
  // Cuando el backend exponga POST /categories, PUT /categories/:id y
  // DELETE /categories/:id, reemplazá estas funciones por llamadas al adapter.
  ...
  return (
    <AdminLayout>
      <div className={styles.page}>
        ...
        {/* LISTA DE CATEGORIAS */}
        {cargando ? (
          <p style={{ padding: '2rem', opacity: 0.5 }}>Cargando categorías...</p>
        ) : categorias.length > 0 ? (
